...
# ============================================
# BRAF-SPECIFIC FILTERING
# ============================================

# Filter for BRAF inhibitors only
print("\nFiltering for BRAF inhibitors...")
braf_dockable = dockable[
    dockable['TARGET'].str.contains('BRAF', case=False, na=False)
].copy()

print(f"  Total dockable compounds: {len(dockable)}")
print(f"  BRAF inhibitors: {len(braf_dockable)}")

# Use BRAF compounds only
dockable_drug_names = set(braf_dockable['DRUG_NAME'])
...
# Curated based on BRAF pathway biology
biology_genes = [
    # BRAF/MAPK pathway (PRIMARY)
    'BRAF', 'KRAS', 'NRAS', 'HRAS', 'MAP2K1', 'MAP2K2', 
    'MAPK1', 'MAPK3', 'RAF1', 'ARAF',
...
# Save main dataset
output_file = DATA_PROCESSED / "caspid_braf_integrated_dataset.csv"
final_dataset.to_csv(output_file, index=False)
print(f"\n✓ Main dataset: {output_file}")

# Save gene list
gene_list_file = DATA_PROCESSED / "braf_selected_genes.txt"
...
print(f"✓ Gene mapping: {gene_map_file}")
...
print(f"✓ Summary: {DATA_PROCESSED / 'integration_summary.csv'}")
