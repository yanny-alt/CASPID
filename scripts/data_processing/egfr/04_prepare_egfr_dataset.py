#!/usr/bin/env python3
"""
FINAL INTEGRATION: Create EGFR-specific CASPID dataset
Combines: Drug response + Cell line matching + Transcriptomics + Compounds

EGFR PRIMARY VALIDATION VERSION
- 8 pure EGFR inhibitors (cleaned dataset)
- ~711 matched cell lines
- ~102 transcriptomic features (EGFR/RTK pathway genes + data-driven)
...
print("="*70)
print("EGFR DATASET INTEGRATION - CLEANED")
print("="*70)
print("\nEGFR primary validation - 8 pure EGFR inhibitors")
...
print("\n" + "="*70)
print("2. FILTERING FOR PURE EGFR INHIBITORS")
print("="*70)

# Filter for MEK inhibitors only
print("\nFiltering for PURE EGFR inhibitors (removing multi-kinase)...")

# CRITICAL: List of 8 PURE EGFR drugs
pure_egfr_drugs = [
    'Afatinib',      
    'AZD3759',        
    'Erlotinib',    
...
print(f"  Pure EGFR drugs to include: {len(pure_egfr_drugs)}")
for drug in sorted(pure_egfr_drugs):
    print(f"    - {drug}")
    
egfr_dockable = dockable[
    dockable['DRUG_NAME'].isin(pure_egfr_drugs)
].copy()

print(f"\n  Total dockable compounds: {len(dockable)}")
print(f"  Pure EGFR inhibitors: {len(egfr_dockable)}")

# Use EGFR compounds only
dockable_drug_names = set(egfr_dockable['DRUG_NAME'])
...
# Save main dataset
output_file = DATA_PROCESSED / "caspid_egfr_integrated_dataset.csv"
final_dataset.to_csv(output_file, index=False)
print(f"\n✓ Main dataset: {output_file}")

# Save gene list
gene_list_file = DATA_PROCESSED / "egfr_selected_genes.txt"
...
summary_df.to_csv(DATA_PROCESSED / "egfr_integration_summary.csv", index=False)
print(f"✓ Summary: {DATA_PROCESSED / 'egfr_integration_summary.csv'}")
...
print("\n" + "="*70)
print("EGFR INTEGRATION COMPLETE")
print("="*70)
...
print(f"  ✓ EGFR inhibitors: {len(final_dataset):,} records")
print(f"    - Pure EGFR drugs only (8 inhibitors)")
print(f"  ✓ Transcriptomic features: {len(final_genes)}")
print(f"    - Biology-guided: {len(available_biology_genes)}")
print(f"    - Data-driven: {len(top_correlated)}")

print(f"\n✓ Key EGFR pathway genes included:")
for simple in ['EGFR', 'ERBB2', 'ERBB3', 'KRAS', 'BRAF', 'PIK3CA', 'AKT1', 'MTOR']:
    if simple in gene_mapping:
        print(f"  • {simple}")

print(f"\n✅ READY FOR EGFR DOCKING!")
print(f"\nNext steps:")
print(f"  1. EGFR protein already prepared (PDB: 1M17)")
print(f"  2. Prepare EGFR ligands (8 pure compounds)")
