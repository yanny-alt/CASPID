...
MODELING_DIR = PROJECT_ROOT / "DATA" / "MODELING" / "mek"
...
# Filter for MEK1 protein only
log("\n  Filtering for MEK1 protein...")
docking_df = docking_df[docking_df['protein'] == 'MEK1'].copy()
log(f"  MEK1 docking features: {len(docking_df)} poses")
...
integrated_file = PROCESSED_DIR / "caspid_mek_integrated_dataset.csv"
...
log("\n  Filtering for HIGH confidence MEK drugs...")
high_conf_mek_drugs = ['pd0325901', 'refametinib', 'selumetinib', 'trametinib']
integrated_df = integrated_df[integrated_df['drug_lower'].isin(high_conf_mek_drugs)].copy()
log(f"  MEK samples (4 drugs): {len(integrated_df)}")       
