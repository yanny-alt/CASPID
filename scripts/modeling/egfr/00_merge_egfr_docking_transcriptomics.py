#!/usr/bin/env python3
"""
00_merge_egfr_docking_transcriptomics.py

CRITICAL SCRIPT: Merges EGFR docking features with integrated dataset.
Creates full modeling dataset for EGFR primary validation.
...
MODELING_DIR = PROJECT_ROOT / "DATA" / "MODELING" / "egfr"
...
# Filter for EGFR protein only
log("\n  Filtering for EGFR protein...")
docking_df = docking_df[docking_df['protein'] == 'EGFR'].copy()
log(f"  EGFR docking features: {len(docking_df)} poses")
...
integrated_file = PROCESSED_DIR / "caspid_egfr_integrated_dataset.csv"
...
# Filter for HIGH confidence MEK1 drugs only
log("\n  Filtering for pure EGFR drugs...")
pure_egfr_drugs = [
    'azd3759',
    'afatinib', 
    'erlotinib',
...
integrated_df = integrated_df[integrated_df['drug_lower'].isin(pure_egfr_drugs)].copy()
log(f"  EGFR samples (7 drugs): {len(integrated_df)}")       
