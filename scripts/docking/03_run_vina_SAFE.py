#!/usr/bin/env python3
"""
CRITICAL: AutoDock Vina Consensus Docking
PRODUCTION VERSION - Safe, tested, with dry-run mode

Features:
- Test mode (1 job first)
- Proper path handling (spaces in filenames)
...
RESULTS_DIR = DOCKING_DIR / "VINA_RESULTS"
RESULTS_DIR.mkdir(parents=True, exist_ok=True)

# Log file
LOG_FILE = RESULTS_DIR / f"vina_docking_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
print("="*70)
print("AUTODOCK VINA - CONSENSUS DOCKING (PRODUCTION)")
print("="*70)
log("Starting AutoDock Vina docking pipeline")

# ============================================
# CHECK VINA INSTALLATION
# ============================================

log("\n1. Checking AutoDock Vina installation...")

try:
    result = subprocess.run(['vina', '--version'], capture_output=True, text=True)
    vina_version = result.stdout.strip()
    log(f"✓ {vina_version}")
except FileNotFoundError:
    log("✗ ERROR: AutoDock Vina not found!")
    log("  Install with: conda install -c conda-forge vina")
    exit(1)
...
VINA_PARAMS = {
    'exhaustiveness': 32,  # High thoroughness for publication
...
log(f"\n4. Vina parameters:")
log(f"   Exhaustiveness: {VINA_PARAMS['exhaustiveness']}")
log(f"   Number of modes: {VINA_PARAMS['num_modes']}")
log(f"   Energy range: {VINA_PARAMS['energy_range']} kcal/mol")
log(f"   CPUs per job: {VINA_PARAMS['cpu']}")

# ============================================
# DOCKING FUNCTION (SINGLE JOB)
# ============================================

def run_vina_docking(job):
    """
    Run single Vina docking job with quality control
    ...
    log_file_path = output_dir / f"{ligand_name}_vina.log"
    ...
    # Get receptor file - use Path object for proper handling
    receptor_file = Path(protein_config['receptor_file'])
    
    # Build Vina command - use string paths to handle spaces
    # Note: Vina 1.2.7 doesn't have --log option, output goes to stdout
    cmd = [
        'vina',
        ...
        '--num_modes', str(VINA_PARAMS['num_modes']),
        '--energy_range', str(VINA_PARAMS['energy_range']),
        '--cpu', str(VINA_PARAMS['cpu']),
    ...
    try:
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=2400  # 40 minute timeout per job (conservative)
        )
        
        elapsed = time.time() - start_time
        
        # Save log output (Vina outputs to stdout)
        ...
        if result.returncode == 0:
            ...
            affinity = parse_vina_affinity_from_text(result.stdout)
            ...
            return {
                ...
                'ligand': ligand_name,
                'status': 'FAILED',
                'reason': f'Vina returned error code {result.returncode}',
...
def parse_vina_affinity_from_text(output_text):
    """
    Extract best binding affinity from Vina output text
    Vina 1.2.7 format:
    ...
    try:
        ...
        # Find result table - look for the header line
        for i, line in enumerate(lines):
            if 'mode |   affinity' in line:
                # Skip units line (i+1) and separator line (i+2)
                # First result is at i+3
                if i + 3 < len(lines):
                    result_line = lines[i + 3]
                    # Line format: "   1       -8.611          0          0"
                    parts = result_line.split()
                    if len(parts) >= 2:
                        # Second column is the affinity
...
cpus_per_job = VINA_PARAMS['cpu']
max_parallel = max(1, total_cpus // cpus_per_job)

# Be conservative - use 75% of calculated max
max_parallel = max(1, int(max_parallel * 0.75))

log(f"   System CPUs: {total_cpus}")
log(f"   CPUs per job: {cpus_per_job}")
log(f"   Parallel jobs: {max_parallel} (conservative)")

# Time estimate (20 min per job average for exhaustiveness=32)
...
if __name__ == '__main__':
    ...
    try:
        with Pool(processes=max_parallel) as pool:
            results = pool.map(run_vina_docking, jobs)
    except KeyboardInterrupt:
        log("\n⚠️  Interrupted by user (Ctrl+C)")
        log("   Partial results will be saved")
        results = []  # Will save what we have
    ...
    if results:
        ...
        results_file = RESULTS_DIR / "vina_results_summary.csv"
        results_df.to_csv(results_file, index=False)
        log(f"    ✓ All results: {results_file}")

    # Save successful docking details
    if success:
        success_df = pd.DataFrame(success)
        success_file = RESULTS_DIR / "vina_successful_dockings.csv"
    ...
    if status == "PASS" and len(success) >= 20:
        log(f"\n🎉 Ready for next step!")
        log(f"\nOptional: Run SMINA for validation")
        log(f"  python scripts/docking/04_run_smina.py")
        log(f"\nOr proceed directly to analysis")
    elif status == "PASS":
        log(f"\n✓ Docking complete")
        log(f"  Review results before proceeding")
    else:
